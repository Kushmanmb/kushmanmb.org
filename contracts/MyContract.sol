...
contract multiowned {
    ...
    // temporary state for that operation allocated.
    ...
    function removeOwner(address _owner) onlymanyowners(keccak256(msg.data)) external {
        ...
        clearPending();
        reorganizeOwners(); //make sure m_numOwners is equal to the number of owners and always points to the optimal free slot
    ...
    function confirmAndCheck(bytes32 _operation) internal returns (bool) {
        ...
        if (pending.ownersDone & ownerIndexBit == 0) {
            ...
            if (pending.yetNeeded <= 1) {
                ...
                delete m_pendingIndex[pending.index];
...
// inheritable "property" contract that enables methods to be protected by placing a linear limit (defined by a number of blocks) on a particular resource
...
// the wallet contract. It inherits multiowned and daylimit contracts.
contract Wallet is multiowned, daylimit {
    ...
    // temporary supplementary data (in the case of a new owner's approval).
